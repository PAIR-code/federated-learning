...
type MockitMsg = any;
type MockitCallback = (msg: MockitMsg, ack?: Function) => void|Promise<void>;
...
export class MockitIOServer {
  ...
  on(event: string, listener: (socket: WrappedClient) => void) {
    if (event !== 'connection') {
      throw new Error(`this mock doesn't support non-connection events`);
    }
...
export class MockitIOClient {
  ...
  trigger(event: string, msg: MockitMsg, ack?: Function) {
    if (this.listeners[event]) {
      this.listeners[event](msg, NOOP);
    }
  }

  emit(event: string, message: string, callback: MockitCallback) {
